...
import br.pucrio.vinhos.model.Vinho;
...
public class VinhoDAO extends CommonsDAO {
	...
	public List<Vinho> selecionarPorTipo( String tipo ) {
		EntityManagerFactory factory = HibernateUtil.getEntityManagerFactory();
		EntityManager manager = factory.createEntityManager();
		Query query = manager.createQuery("select v from Vinho v where v.tipo = :tipo order by idVinho");
		query.setParameter("tipo", tipo);
		if( query.getResultList() != null && !query.getResultList().isEmpty()  ) {
			List<Vinho> resultado = (List<Vinho>)query.getResultList();
			manager.close();
			return resultado;
		} 
		manager.close();
